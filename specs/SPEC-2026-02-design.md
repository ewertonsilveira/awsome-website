...
| `.github/workflows/deploy.yml` | **PROTECTED PATH.** GitHub Actions on push to `main`: checkout → setup-node@v4 (Node 24) → `npm ci` → `npm run typecheck` → `npm run build` → `wrangler pages deploy dist/client --project-name=awsome-nz`, all with `working-directory: web-app`. Uses secrets `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`. |
