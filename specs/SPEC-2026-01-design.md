...
| **T12** | 404 catch-all `$.tsx` + `defaultNotFoundComponent` in `router.tsx`. **Inherits the S1/AC-05 direct-hit ownership from T2: add `{ path: '/404' }` to `pages` in `web-app/vite.config.ts` (now possible because `$.tsx` exists) and explicitly confirm `dist/client/404.html` is emitted** (ADR-06) | `web-app/src/routes/$.tsx`, `src/router.tsx`, `web-app/vite.config.ts` | T7 | AC-05 | S |
