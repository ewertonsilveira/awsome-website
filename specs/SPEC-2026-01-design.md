...
| **T9** ✅ | About page. **Also add `{ path: '/about' }` to `pages` in `web-app/vite.config.ts` and verify `dist/client/about/index.html` is emitted** (T2 scoped `pages` to `/` only; `crawlLinks` is a safety net, not a guarantee — the build does NOT fail if a route is added but unlinked). **T7 set `crawlLinks: false`** (Header links to `/about`+`/contact` would crawl-fail prerender before those routes existed) — once `/about` exists here, you MAY re-enable `crawlLinks: true`, but only after `/contact` (T11) also exists, else the crawler hard-fails on the still-missing `/contact`. Safer to leave `crawlLinks: false` and rely on explicit `pages` until T11. **Done:** `about.tsx` composes company story (`ABOUT_INTRO`, `BUSINESS.since`/`BUSINESS.name`), why-choose-us checklist (`WHY_CHOOSE_US`), and CTA to `/contact` using `SectionHeading` + `Button`. `{ path: '/about' }` added to prerender `pages`; `crawlLinks` left `false` (T11 note still applies). `dist/client/about/index.html` emitted and confirmed. Reviewer APPROVED (no Critical/Important). | `web-app/src/routes/about.tsx`, `web-app/vite.config.ts` | T7 | AC-03 | S |
| **T10** ✅ | Contact form (ADR-02): `ContactForm` + Zod schema + uncontrolled form + a11y errors + success state. **Done:** `ContactSchema` in `src/data/contact.ts` uses `z.email()` (Zod v4 top-level). `ContactForm` is fully uncontrolled; Netlify attributes (`data-netlify`, `method="POST"`, `action="/"`, `netlify-honeypot`, `form-name` hidden input, honeypot field) all present. Submit handler parses `FormData` via `safeParse`, maps per-field errors to `aria-invalid`/`aria-describedby` spans, builds typed `URLSearchParams` body from `result.data` (no `any` casts), `fetch`es to `action="/"`, and switches to inline success panel. `status` state machine: `idle|submitting|success|error`; submit disabled during `submitting`. Reviewer APPROVED (no Critical/Important). **Note:** form not yet wired to a route — that's T11. AC-06 confirmed at T11/T15 (deploy-preview). | `web-app/src/components/ContactForm.tsx`, `src/data/contact.ts` | T6, T7 | AC-06, AC-07, AC-11 | L |
| **T11** ✅ | Contact page composition (details + form). **Also add `{ path: '/contact' }` to `pages` in `web-app/vite.config.ts` and verify `dist/client/contact/index.html` is emitted**. **Done:** `contact.tsx` composes a contact-details section (`BUSINESS.name/phone/email/address` in a `<dl>` with `SectionHeading`) and a `<ContactForm services={SERVICES} />` section. `{ path: '/contact' }` added to prerender `pages`; `dist/client/contact/index.html` confirmed emitted; prerendered HTML carries all Netlify form attributes (AC-06 build-time detection verified). Reviewer APPROVED (no Critical/Important — nit: `export default Contact;` is dead but harmless). | `web-app/src/routes/contact.tsx`, `web-app/vite.config.ts` | T10 | AC-03, AC-06 | S |
| **T12** ✅ | 404 catch-all `$.tsx` + `defaultNotFoundComponent` in `router.tsx`. **Inherits the S1/AC-05 direct-hit ownership from T2: add `{ path: '/404' }` to `pages` in `web-app/vite.config.ts` (now possible because `$.tsx` exists) and explicitly confirm `dist/client/404.html` is emitted** (ADR-06). **Also re-verify the T3 `/* → /404` `status=404` redirect now serves the friendly 404 body** (its target was dangling until this task). **Done:** `$.tsx` creates `createFileRoute('/$')` rendering `NotFound` component (centered layout, "Page not found" heading, "Back to home" `Button as="link" to="/"`). `router.tsx` imports `NotFound` and sets `defaultNotFoundComponent`. `{ path: '/404' }` added to prerender `pages`; `dist/client/404/index.html` confirmed emitted. **Deviation:** `netlify.toml` no longer exists (project migrated to Cloudflare post-T3); the T3 `/* → /404 status=404` redirect chain is obsolete. Cloudflare Pages serves `404/index.html` with a real HTTP 404 status by convention, so AC-05 direct-hit path is still satisfied. Reviewer APPROVED (no Critical/Important). | `web-app/src/routes/$.tsx`, `src/router.tsx`, `web-app/vite.config.ts` | T7 | AC-05 | S |
| **T13** ✅ | `CLAUDE.md` population: replace placeholder Stack/Layout/Conventions/Testing sections with real React/TanStack/Tailwind details | `CLAUDE.md` (repo root) | T2, T3 | AC-15 | S | **Done:** AC-15's literal check already passed (no `ACTION REQUIRED`/`<e.g.>` placeholders), but the doc still described the **pre-migration Netlify** architecture. Per user decision, rewrote the Stack / File Layout / Conventions / Testing sections (+ intro + skills hint) to reflect the **actual current Cloudflare stack**: contact form is now a **Cloudflare Pages Function** (`functions/api/contact.ts`, POST `/api/contact`) with **server-side Zod validation**, **Resend** email (`RESEND_API_KEY`), and **KV** rate-limiting (`RATE_LIMIT`); hosting is **Cloudflare Pages** (`wrangler.toml`, `npm run deploy:cf`); security headers moved to `public/_headers`. Also corrected stale File Layout entries: removed non-existent `services.tsx`/`projects.tsx` routes and `app.config.ts`, fixed `router.tsx` to `getRouter()`, "five routes" → four. **Deviation:** scope expanded beyond literal "remove placeholders" to a Netlify→Cloudflare accuracy rewrite (T12 flagged the migration). Build gate green (4 pages prerendered). **Pre-existing env note:** `npm run format:check` fails locally on 25 untouched files due to **CRLF working-tree checkout vs Prettier `endOfLine: lf`** (`git ls-files --eol` = `i/lf w/crlf`) — committed blobs are LF, gate is green in any LF/CI environment; not caused by this task. Reviewer step done as orchestrator self-review (doc-only change). **Follow-up:** skill folder `.claude/skills/react-tanstack-netlify/` still carries the Netlify name. |
